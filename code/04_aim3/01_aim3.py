...
# ===== CONFIG =====
DATA_DIR = str(Path.home() / "Downloads")  # 전처리 단계 생략, csv 바로 사용 (로컬 전용)
...
r"""## 2. B2 모형 — Aim2와 동일한 5-임계값 부분비례오즈 구조
...
r"""### 2-1. 검증 셀 — 전체표본 1회 적합 시 확정 계수와 근사한지 확인
...
r"""### 2-2. 진단 — 왜 P(Y≥1)/P(Y≥2)만 크게 어긋나는가
...
r"""### 3-1. 단계변화량 일치도 (MAE, Spearman) — 3순위 지표"""
...
print('[24주]')
print(eval_change_agreement(delta24))
print('[12주]')
print(eval_change_agreement(delta12))

r"""### 3-2. 3범주(악화/안정/호전) 가중카파 — 2순위 지표 (안정=Δ0만, 확정)"""
...
print('[24주]')
print(weighted_kappa_3cat(delta24))
print('[12주]')
print(weighted_kappa_3cat(delta12))
...
try:
    ...
    subprocess.run([_sys.executable, "-m", "pip", "install", "koreanize-matplotlib"], check=True)
    import koreanize_matplotlib

import matplotlib
matplotlib.use('Agg')  # 비대화형 실행 환경(스크립트로 직접 실행) — plt.show()가 창을 띄우고 무한대기하는 것 방지
import matplotlib.pyplot as plt
...
def plot_confusion_heatmaps(delta_df, span_label):
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / f'confusion_heatmaps_{span_label}.png', dpi=150)
    plt.close(fig)

plot_confusion_heatmaps(delta24, '24주')
plot_confusion_heatmaps(delta12, '12주')

r"""### 3-2-보완. EDA — 왜 3범주 가중카파가 낮은가 (델타 분포 + 카파역설 진단)"""
...
def delta_distribution_summary(delta_df, span_label):
    ...
    for col in cols:
        ...
        print(f'\n[{col}]  n={vc.sum()}')
        print(vc)
    print(delta_df[cols].describe().T)
...
r"""### 3-3. 악화 이진판정 — 1순위 지표 (사전확정 준거: TDS≥2)
...
for label, (col, thr) in SENS_CRITERIA.items():
    print(f'--- {label} (24주) ---')
    print(binary_worsening_eval(delta24, col, thr))

r"""### 3-4. 지표간 비교 — 부트스트랩 CI(MAE 차이) + McNemar(이진판정 차이)"""
...
r"""### 3-5. Bland-Altman (단계변화, 단위 동일한 것만)"""
...
def bland_altman(delta_df, m='B2', span_label='24주'):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(FIG_DIR / f'bland_altman_{m}_{span_label}.png', dpi=150)
    plt.close()
...
r"""### 4-보완. EDA — SRM·SRC 분산 분해: 기저치분산 vs 변화량분산, 어느 쪽이 압축됐나"""
...
def variance_decomposition_table(df, visit='V7', span_label='24주'):
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / f'variance_decomposition_{span_label}.png', dpi=150)
    plt.close(fig)
...
r"""## 5. 다절 — 반복측정 혼합모형 (R: lme4 + lmerTest)
...
r"""### 5-0. 장기 포맷(long-format) 데이터 → CSV 내보내기
...
r"""### 5-1. 공분산 구조 민감도분석 (random slope / AIC 비교)
