...
def load_domains(data_dir):
    KEY_DOMAINS = ["dm", "pf", "qs", "lb", "ss", "nv",
                   "mh", "cm", "vs", "sv", "xd", "ds", "ft"]

    # 대용량 파일은 필요한 컬럼만 읽기
    COLS = {
        "qs": ["STUDYID", "USUBJID", "QSTESTCD", "QSTEST", "QSSTRESN",
               "QSORRES", "VISITNUM", "VISIT", "VISITDY", "EPOCH"],
        "lb": ["STUDYID", "USUBJID", "LBTESTCD", "LBTEST", "LBSTRESN",
               "LBORRES", "LBCAT", "VISITNUM", "VISITDY"],
        "nv": ["STUDYID", "USUBJID", "NVTESTCD", "NVTEST", "NVSTRESN",
               "NVORRES", "VISITNUM", "VISITDY"],
    }

    if not os.path.exists(data_dir):
        print(f"[경고] 경로가 존재하지 않습니다: {data_dir}")
        return {}

    domains = {}
    for name in KEY_DOMAINS:
        fpath = os.path.join(data_dir, f"{name}.csv")
        if not os.path.exists(fpath):
            print(f"  [없음] {name}.csv")
            continue
        size_mb = os.path.getsize(fpath) / 1024 / 1024
        print(f"  읽는 중: {name}.csv ({size_mb:.1f} MB)...", end=" ", flush=True)
        try:
            usecols = COLS.get(name, None)
            if usecols:
                # 실제 존재하는 컬럼만 필터
